...
import {
  createPgComponent as createBasePgComponent,
  Options,
...
export async function createPgComponent(
  ...
  let databaseUrl: string | undefined = await config.getString(
    `${dbPrefix}_PG_COMPONENT_PSQL_CONNECTION_STRING`
  );
  if (!databaseUrl) {
    const dbUser = await config.requireString(
      `${dbPrefix}_PG_COMPONENT_PSQL_USER`
    );
    const dbDatabaseName = await config.requireString(
      `${dbPrefix}_PG_COMPONENT_PSQL_DATABASE`
    );
    const dbPort = await config.requireString(
      `${dbPrefix}_PG_COMPONENT_PSQL_PORT`
    );
    const dbHost = await config.requireString(
      `${dbPrefix}_PG_COMPONENT_PSQL_HOST`
    );
    const dbPassword = await config.requireString(
      `${dbPrefix}_PG_COMPONENT_PSQL_PASSWORD`
    );
